...
{
    using BookLibrary;
    using G_NET_12_OOP01.TicketBookingSystem;
    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            #region Part 2
            // Q1 : Enum (TicketTypes)
            // Q2 : Struct (SeatLocation)

        
            
            Console.Write("Enter Movie Name: ");
            string movieName = Console.ReadLine();
            Console.Write("Enter Ticket Type (0 = Standard, 1 = Premium, 2 = VIP): ");
            int ticketTypeInput = int.Parse(Console.ReadLine());
            if (ticketTypeInput < 0 || ticketTypeInput > 2)
            {
                Console.WriteLine("Invalid ticket type. Defaulting to Standard.");
                ticketTypeInput = 0;
            }
            ...
            Ticket ticketBooking = new Ticket(movieName, (TicketTypes)ticketTypeInput, new SeatLocation(char.ToUpper(seatRow), seatNumber), price);

            Console.WriteLine(ticketBooking.PrintTicket(discountAmount));
